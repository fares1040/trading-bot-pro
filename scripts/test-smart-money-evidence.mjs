...
test('multiple fresh observations can only reach possible', () => { const r = classifySmartMoneyEvidence({ observations:[obs(),obs(),obs()] }); assert.equal(r.classification, 'POSSIBLE'); });
...
test('missing notional does not become zero-derived proof', () => { const r = classifySmartMoneyEvidence({ observations:[{...obs(), notional:null},{...obs(), notional:null},{...obs(), notional:null}] }); assert.equal(r.notionalObserved, 0); assert.equal(r.classification, 'POSSIBLE'); });
