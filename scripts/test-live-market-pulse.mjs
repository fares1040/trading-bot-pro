...
test('handles insufficient history without fabrication', () => { const pulse = buildLiveMarketPulse(base(101, 1100)); assert.equal(pulse.symbol, 'AAPL'); assert.equal(pulse.price, 101); assert.equal(pulse.priceChangePercent, null); assert.equal(pulse.priceAccelerationPercent, null); assert.equal(pulse.accelerationState, 'INSUFFICIENT_DATA'); assert.equal(pulse.evidence.length, 0); });
