...
export async function GET(request) {
    ...
    try {
        ...
        if (!data.chart || !data.chart.result) return NextResponse.json({ error: "لا توجد بيانات" }, { status: 404 });
        ...
        const analysisText = `تحليل سهم ${symbol.toUpperCase()}: السعر: ${price.toFixed(2)} | القرار: ${isEntrySuitable ? "مناسب للدخول ✅" : "انتظر الفرصة ❌"}`;

        // إذا كان مناسباً، أرسل التنبيه
        if (isEntrySuitable) {
