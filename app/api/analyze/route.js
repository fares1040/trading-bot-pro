...
export async function POST(request) {
  try {
    ...
    // 🤖 ضع توكن بوت تيليجرام ورقم الآيدي هنا مباشرة للتجربة الفورية
    ...
    // 🤖 إضافة تحليل الذكاء الاصطناعي وخريطة السيولة اللحظية (Order Flow / Heatmap)
...
• القرار النهائي: ${isSuitable ? 'هدف مؤكد ونموذج مكتمل 🔥' : isEarlyAlert ? '⏳ إنذار استباقي مبكر - يقترب من الاكتمال 🎯' : 'قيد المراقبة الاستباقية 🛡️'}`;
    ...
    // 1️⃣ إرسال تنبيه ديسكورد (Discord Webhook)
    ...
    // 2️⃣ إرسال تنبيه تليجرام (Telegram Bot API) مباشرة من الكود
    if (telegramToken && telegramToken !== "8822034470:AAEbooViT3tdkkQqt2lx86GZBWipYUq0MgA" && telegramChatId && (isSuitable || isEarlyAlert) && !isInCooldown) {
      try {
