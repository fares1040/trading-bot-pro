...
async function probe(url) {
  ...
  try {
    ...
    return { ok: response.ok && json?.success !== false, status: response.status, latencyMs: Date.now() - started, error: response.ok ? null : (json?.error || `HTTP ${response.status}`) };
  } catch (error) {
    return { ok: false, status: null, latencyMs: Date.now() - started, error: error?.message || 'probe failed' };
  }
}
...
export async function GET(request) {
  ...
  const [health, indices, stocks] = await Promise.all([
    ...
    probe(`${origin}/api/indices`),
    probe(`${origin}/api/stocks?symbol=SOFI`),
  ]);

  const checks = { health: health.ok, indices: indices.ok, stocks: stocks.ok };
  ...
  return NextResponse.json({
    ...
    checks,
    latencyMs: { health: health.latencyMs, indices: indices.latencyMs, stocks: stocks.latencyMs },
    failures: [
