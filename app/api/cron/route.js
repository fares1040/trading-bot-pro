...
/* =========================
   VOLUME
========================= */
function calculateVolumeMetrics(volume, volumes) {
    const avgVolume20 = average(volumes.slice(-21, -1));
    const relativeVolume = avgVolume20 > 0
        ? volume / avgVolume20
        : 0;
    const volumeScore = clamp(45 +
        (relativeVolume - 1) *
            35);
    return {
        avgVolume20,
...
function calculateTrendScore(price, sma20, sma50) {
    ...
    const distance = ((price - sma50) /
        sma50) *
        100;
    score += clamp(distance * 2, -20, 20);
    return clamp(score);
}
/* =========================
   BREAKOUT
========================= */
function calculateBreakout(price, prior20High) {
    if (!price ||
        !prior20High) {
        return {
    ...
    const distancePercent = ((prior20High - price) /
        prior20High) *
        100;
    let score = 25;
...
/* =========================
   CLUSTER
========================= */
function calculateCluster(price, highs, lows, relativeVolume) {
    ...
    const rangePercent = ((recentHigh - recentLow) /
        price) *
        100;
    const isCluster = rangePercent <= 3.5 &&
        relativeVolume >= 1.1;
    let score = 25;
    if (rangePercent <= 2) {
        score = 100;
...
function calculateRiskReward(price, support, resistance) {
    ...
    return {
        ...
        riskPercent: round((risk / price) *
            100),
        rewardPercent: round((reward / price) *
            100),
        score,
    };
}
/* =========================
   FINAL SCORE
========================= */
function calculateTechnicalScore({ momentumScore, volumeScore, trendScore, breakoutScore, rsiScore, squeezeScore, clusterScore, riskScore, resistancePenalty, }) {
    const raw = momentumScore * 0.15 +
        volumeScore * 0.15 +
        trendScore * 0.15 +
        breakoutScore * 0.15 +
        rsiScore * 0.10 +
...
function buildAnalytics(meta, quote) {
    ...
    const volumeMetrics = calculateVolumeMetrics(volume, volumes);
    const prior20High = Math.max(...closes.slice(-21, -1));
    const prior20Low = Math.min(...lows.slice(-21, -1));
    const momentum = calculateMomentum(closes);
    const trendScore = calculateTrendScore(price, sma20, sma50);
    const breakout = calculateBreakout(price, prior20High);
    const rsiScore = calculateRsiScore(rsi);
    const bollinger = calculateBollinger(closes);
    const cluster = calculateCluster(price, highs, lows, volumeMetrics.relativeVolume);
    const riskReward = calculateRiskReward(price, prior20Low, prior20High);
    const resistanceDistancePercent = ((prior20High -
        price) /
        price) *
        100;
    let resistancePenalty = 0;
    if (resistanceDistancePercent >=
        0 &&
        resistanceDistancePercent <
            1) {
        resistancePenalty = 12;
    }
    else if (resistanceDistancePercent < 2) {
        resistancePenalty = 7;
    }
    const technicalScore = calculateTechnicalScore({
        momentumScore: momentum.score,
        volumeScore: volumeMetrics.volumeScore,
        trendScore,
        breakoutScore: breakout.score,
        rsiScore,
        squeezeScore: bollinger.score,
        clusterScore: cluster.score,
        riskScore: riskReward.score,
        resistancePenalty,
    });
    const signal = getSignal(technicalScore);
    const bullishStructure = price > sma20 &&
        price > sma50 &&
        sma20 > sma50;
    return {
        price: round(price),
        ...
        relativeVolume: round(volumeMetrics.relativeVolume),
        rsi: rsi == null
            ? null
            : round(rsi),
        sma20: round(sma20),
        ...
        volumeScore: Math.round(volumeMetrics.volumeScore),
        trendScore: Math.round(trendScore),
        breakoutScore: Math.round(breakout.score),
        squeeze: bollinger.squeeze,
        bollingerBandwidth: round(bollinger.bandwidth),
        squeezeScore: Math.round(bollinger.score),
        cluster: cluster.isCluster,
        clusterRangePercent: round(cluster.rangePercent),
        clusterScore: Math.round(cluster.score),
        resistanceDistancePercent: round(resistanceDistancePercent),
        ...
        rewardPercent: riskReward.rewardPercent,
        technicalScore,
...
async function fetchStock(symbol) {
    ...
    const quoteType = result.meta?.quoteType ||
        result.meta?.instrumentType;
    if (quoteType &&
        quoteType !== 'EQUITY') {
        return null;
    }
    const analytics = buildAnalytics(result.meta, result.indicators
        ?.quote?.[0] || {});
    if (!analytics) {
        return null;
    }
...
/* =========================
   CANDIDATE GATE
========================= */
function qualifies(analytics) {
    if (!analytics) {
        return false;
    }
    const priceOk = analytics.price > 0 &&
        analytics.price < 100;
    const volumeOk = analytics.volume >=
        100000;
    const scoreOk = analytics.technicalScore >=
        75;
    const confirmation = analytics.relativeVolume >=
        1.1 ||
        analytics.squeeze ||
        analytics.cluster ||
...
/* =========================
   BUILD OPPORTUNITY
========================= */
function buildOpportunity(stock) {
    const m = stock.analytics;
    let status = '📊 فرصة فنية';
    let reason = 'توافق عدة عوامل فنية معاً.';
    if (m.squeeze &&
        m.cluster &&
        m.bullishStructure) {
        status =
    ...
    */
    return {
        ...
        target_price: m.targetPrice,
        stop_loss: m.stopLoss,
        confidence: m.technicalScore,
        reason: `${reason} | RSI ${m.rsi ?? 'N/A'} | RVOL ${m.relativeVolume}x | SMA50 ${m.sma50 ?? 'N/A'}`,
        sector: symbolSectors[stock.symbol] || 'عام',
        /*
        ...
        */
        risk_reward: m.riskReward,
        risk_percent: m.riskPercent,
        reward_percent: m.rewardPercent,
        technical: {
...
/* =========================
   DUPLICATE CHECK
========================= */
async function isDuplicate(supabase, ticker) {
    if (!supabase) {
        return false;
    }
...
/* =========================
   SAVE
========================= */
async function saveOpportunity(supabase, opportunity) {
    if (!supabase) {
        return false;
    }
    if (await isDuplicate(supabase, opportunity.ticker)) {
        return false;
    }
    /*
      IMPORTANT:
    ...
    */
    const dbRow = {
        ...
        volume_status: opportunity.volume_status,
    ...
    if (error) {
        console.error('auto_signals insert error:', error.message);
        return false;
    }
    return true;
}
/* =========================
   REPORT
========================= */
function buildReport(opportunities) {
    let report = '🔥 *Trading Bot Pro V4 — Radar* 🔥\n' +
    ...
    try {
        ...
        */
        for (let i = 0; i < symbols.length; i += BATCH_SIZE) {
            const batch = symbols.slice(i, i +
                BATCH_SIZE);
            const results = await Promise.allSettled(batch.map(symbol => fetchStock(symbol)));
            results.forEach(result => {
                ...
                if (!qualifies(stock.analytics)) {
                    return;
                }
                detected.push(buildOpportunity(stock));
            });
        }
        /*
          الأقوى أولاً
        */
        detected.sort((a, b) => (b.confidence ||
            0) -
            (a.confidence ||
                0));
        /*
          حفظ أفضل 10 فقط
        */
        for (const opportunity of detected.slice(0, 10)) {
            const saved = await saveOpportunity(supabase, opportunity);
            if (saved) {
                newlySaved.push(opportunity);
            }
        }
        /*
          ...
          وليس كل Cron
        */
        await sendNotifications(newlySaved);
        return NextResponse.json({
            success: true,
            market_active: true,
            ny_time: market.nyTime,
            ...
            newly_saved: newlySaved.length,
            data: detected.slice(0, 10),
            ...
            limitations: 'لا توجد Options Chain أو Dark Pool أو Institutional Flow فعلية من مصدر البيانات الحالي.',
            timestamp: new Date().toISOString(),
        });
    }
    catch (error) {
        console.error('Cron radar error:', error);
        return NextResponse.json({
            success: false,
            error: 'تعذر تشغيل الرادار الآلي حالياً',
