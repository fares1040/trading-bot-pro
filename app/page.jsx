'use client';
import { useState, useEffect } from 'react';

export default function Home() {
  const [ticker, setTicker] = useState('');
  const [newTicker, setNewTicker] = useState('');
  const [watchData, setWatchData] = useState([]);
  
  // تحميل القائمة من ذاكرة المتصفح، وإذا كانت فارغة استخدم القائمة الافتراضية
  const [watchlist, setWatchlist] = useState(() => {
    if (typeof window !== 'undefined') {
      const saved = localStorage.getItem('myWatchlist');
      return saved ? JSON.parse(saved) : ['HURA', 'KULR', 'BYRN', 'BJSX', 'PODC', 'SPSC', 'MRAM', 'NOK', 'OPI'];
    }
    return ['HURA', 'KULR', 'BYRN', 'BJSX', 'PODC', 'SPSC', 'MRAM', 'NOK', 'OPI'];
  });

  // حفظ القائمة في ذاكرة المتصفح كلما تغيرت
  useEffect(() => {
    localStorage.setItem('myWatchlist', JSON.stringify(watchlist));
  }, [watchlist]);

  useEffect(() => {
    const fetchWatchlist = async () => {
      const data = await Promise.all(watchlist.map(async (s) => {
        try {
          const res = await fetch(`/api/analyze?symbol=${s}`);
          const json = await res.json();
          return { ...json, stopLoss: (json.currentPrice * 0.97).toFixed(2) };
        } catch { return { symbol: s, currentPrice: 'Error' }; }
      }));
      setWatchData(data);
    };
    fetchWatchlist();
  }, [watchlist]);

  return (
    <div style={{ backgroundColor: '#121212', color: '#fff', padding: '20px', fontFamily: 'monospace' }}>
      <h1 style={{ textAlign: 'center', color: '#00ff41' }}>غرفة العمليات - نظام السنايبر</h1>
      
      {/* قسم الإضافة */}
      <div style={{ marginTop: '20px', marginBottom: '20px' }}>
        <input onChange={(e) => setNewTicker(e.target.value)} value={newTicker} placeholder="إضافة سهم..." style={{ padding: '10px', width: '80%', color: '#000' }} />
        <button onClick={() => { if(newTicker) { setWatchlist([...watchlist, newTicker.toUpperCase()]); setNewTicker(''); } }} style={{ padding: '10px', width: '19%' }}>إضافة</button>
      </div>

      <table style={{ width: '100%', marginTop: '20px', borderCollapse: 'collapse' }}>
        <thead>
          <tr style={{ color: '#888', borderBottom: '2px solid #333' }}>
            <th>السهم</th>
            <th>السعر</th>
            <th>SL</th>
            <th>التحليل (اضغط لمعرفة المزيد)</th>
            <th>حذف</th>
          </tr>
        </thead>
        <tbody>
          {watchData.map((s, i) => (
            <tr key={i} style={{ borderBottom: '1px solid #222' }}>
              <td>{s.symbol}</td>
              <td>{s.currentPrice}</td>
              <td style={{ color: '#ff4444', fontWeight: 'bold' }}>{s.stopLoss}</td>
              {/* عرض التحليل بشكل أوضح */}
              <td onClick={() => alert(s.analysis)} style={{ cursor: 'pointer', color: '#00ff41' }}>عرض التحليل</td>
              <td>
                <button onClick={() => setWatchlist(watchlist.filter(item => item !== s.symbol))} style={{ color: 'red', background: 'none', border: 'none', cursor: 'pointer' }}>حذف</button>
              </td>
            </tr>
          ))}
        </tbody>
      </table>
    </div>
  );
}
