...
function buildAlertFromInputs(inputs, timestamp = nowISO()) {
  ...
  for (const item of inputs) {
    ...
    const primaryDataCompleteness = sources.opportunityRanking?.dataCompleteness ??
                                     sources.swingIntelligence?.dataCompleteness ??
                                     sources.pennyIntelligence?.dataCompleteness ??
                                     sources.tradePlan?.dataCompleteness;
    const dataCompleteness = primaryDataCompleteness != null
      ? clamp(primaryDataCompleteness)
...
function isAvoidSignal(alert) {
  if (!alert || typeof alert !== 'object') return false;
  const signal = alert.signal;
  if (signal != null && String(signal).toUpperCase() === 'AVOID') return true;
  const evidence = Array.isArray(alert.evidence) ? alert.evidence.join(' ') : '';
  if (evidence && evidence.toLowerCase().includes('avoid')) return true;
  return false;
}

function selectTop3Alerts(alerts, options = {}) {
  const limit = options.limit != null ? options.limit : 3;
  const excludeAvoid = options.excludeAvoid !== false;

  if (!Array.isArray(alerts) || alerts.length === 0) {
    return [];
  }

  let candidates = alerts;

  if (excludeAvoid) {
    candidates = candidates.filter(a => !isAvoidSignal(a));
  }

  const ranked = rankAlerts(candidates, 'priority', 'desc');

  return ranked.slice(0, limit).map((alert, index) => ({
    ...alert,
    rank: index + 1,
    notificationId: `TOP3:${alert.id}:${alert.timestamp}`,
  }));
}

function buildTop3NotificationPayload(alerts, options = {}) {
  const timestamp = options.timestamp || nowISO();
  const top3 = selectTop3Alerts(alerts, { limit: 3, excludeAvoid: true });
  const suppressed = Array.isArray(alerts)
    ? alerts.filter(a => isAvoidSignal(a)).length
    : 0;
  ...
  return {
    ...
    top3: top3,
    count: top3.length,
    suppressedAvoid: suppressed,
    disclaimer: 'Top-3 Alert Notifications are in-app/browser analytical alerts only — not a trade recommendation. AVOID candidates are suppressed. Missing evidence remains null.',
    limitations: top3.length === 0
      ? ['No qualifying Top-3 opportunities available']
      : (suppressed > 0
          ? [`${suppressed} AVOID candidate(s) suppressed from Top-3`]
          : []),
  };
}

const TOP3_CONSTANTS = Object.freeze({
  TOP_LIMIT: 3,
  EXCLUDE_AVOID: true,
});
