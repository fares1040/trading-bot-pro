...
import { detectEarlyExplosion } from './early-explosion-radar.js';
import { aggregateLiveFlow } from './live-flow-aggregator.js';
import { classifySmartMoneyEvidence } from './smart-money-evidence.js';
import { fetchUnusualWhalesFlow, isUnusualWhalesConfigured } from './unusual-whales-flow-provider.js';
...
async function resolveFlowInput(symbol, flowInput = {}) {
  if (flowInput?.provided) return flowInput;

  if (isUnusualWhalesConfigured()) {
    try {
      return await fetchUnusualWhalesFlow(symbol);
    } catch (error) {
      return {
        observations: [],
        attributionEvidence: [],
        dataQuality: 'UNAVAILABLE',
        providerError: 'Unusual Whales flow request failed',
      };
    }
  }

  return { observations: [], attributionEvidence: [], dataQuality: 'UNAVAILABLE' };
}
...
async function processSymbol(symbol, flowInput = {}) {
  ...
  const baseEntry = {
    ...
    pulse: {
      ...
      spreadPercent: pulse.spreadPercent,
    },
    opportunity: {
  ...
  const resolvedFlow = await resolveFlowInput(symbol, flowInput);
  const enriched = enrichLiveOpportunityWithFlow(
    baseEntry,
    resolvedFlow.observations,
    resolvedFlow.attributionEvidence,
  );
  const earlyExplosion = detectEarlyExplosion({
    ...enriched,
    optionsFlow: enriched.optionsFlow,
  });

  if (resolvedFlow.dataQuality && resolvedFlow.dataQuality !== enriched.flow.dataQuality) {
    enriched.flow = { ...enriched.flow, providerDataQuality: resolvedFlow.dataQuality };
  }
  if (resolvedFlow.providerError) {
    enriched.flow = { ...enriched.flow, providerError: resolvedFlow.providerError };
  }
  ...
  return {
    ...
    evidence: [
      ...
      ...(earlyExplosion.state !== 'QUIET' ? [`EARLY_EXPLOSION: ${earlyExplosion.state}`] : []),
    ],
  };
}
...
export async function processLiveOpportunities(rawSymbols, options = {}) {
  ...
  for (const symbol of limited) {
    try {
      const suppliedFlow = Object.prototype.hasOwnProperty.call(flowBySymbol, symbol)
        ? { ...normalizeFlowInput(flowBySymbol, symbol), provided: true }
        : {};
      const entry = await processSymbol(symbol, suppliedFlow);
      data.push(entry);
    } catch (error) {
      errors.push({ symbol, error: 'Live market data processing failed' });
    }
  }
  ...
  return {
    ...
    data,
    errors,
    disclaimer: 'Live opportunity radar is derived from polled Yahoo data. Optional options-flow evidence is sourced from Unusual Whales only when configured. Not a trading recommendation.',
  };
}

export { clearHistory, getHistory, updateHistory, normalizeFlowInput };
