...
// ============================================================================
// PRICE BREADTH SCORING
// Percentage of universe symbols with positive daily change.
// This is real breadth data computed from available universe data.
// No fabrication - missing changePercent is excluded from calculation.
// ============================================================================

export function scorePriceBreadth(universe) {
  const reasons = [];
  const flags = [];

  if (!Array.isArray(universe) || universe.length === 0) {
    return { score: null, flags, reasons, sampleSize: 0, dataAvailability: 'UNAVAILABLE' };
  }
  ...
  if (changes.length === 0) {
    return { score: null, flags, reasons, sampleSize: universe.length, dataAvailability: 'UNAVAILABLE' };
  }

  const positivePct = (positive / changes.length) * 100;
  const negativePct = (negative / changes.length) * 100;
  const avgChange = changes.reduce((a, b) => a + b, 0) / changes.length;

  // Breadth score: % positive maps to 0-100
  const score = clamp(positivePct);

  reasons.push('Price breadth: ' + positive + '/' + changes.length + ' symbols positive (' + positivePct.toFixed(0) + '%)');
  reasons.push('Avg price change ' + avgChange.toFixed(2) + '% across ' + changes.length + ' symbols');

  if (positivePct >= 60) flags.push('BROAD_POSITIVE');
  else if (positivePct <= 40) flags.push('BROAD_NEGATIVE');
  else flags.push('MIXED_BREADTH');

  if (negativePct >= 70) flags.push('SEVERE_BREADTH_DECLINE');
...
// ============================================================================
// RSI DISTRIBUTION SCORING
// Distribution of RSI values across the universe.
// Identifies overbought/oversold concentration.
// No fabrication - missing RSI is excluded from calculation.
// ============================================================================

export function scoreRSIDistribution(universe) {
  const reasons = [];
  const flags = [];
  ...
  const overboughtPct = (overbought / rsiValues.length) * 100;
  const oversoldPct = (oversold / rsiValues.length) * 100;

  // RSI distribution score: avg RSI maps to 0-100
  const score = clamp(avgRSI);

  reasons.push('RSI distribution: avg ' + avgRSI.toFixed(1) + ' across ' + rsiValues.length + ' symbols');
  reasons.push('Overbought (>=70): ' + overbought + ' (' + overboughtPct.toFixed(0) + '%), Oversold (<=30): ' + oversold + ' (' + oversoldPct.toFixed(0) + '%)');

  if (overboughtPct >= 30) flags.push('WIDE_OVERBOUGHT');
  if (oversoldPct >= 30) flags.push('WIDE_OVERSOLD');
  if (avgRSI >= 60) flags.push('ELEVATED_RSI');
  if (avgRSI <= 40) flags.push('DEPRESSED_RSI');
  ...
  return {
    ...
    avgRSI: round(avgRSI, 1),
    overboughtPct: round(overboughtPct, 1),
    oversoldPct: round(oversoldPct, 1),
    dataAvailability: 'AVAILABLE',
  };
}
...
// Groups components into evidence families for structured explanation.
// Families: MARKET_STRUCTURE, BREADTH, INTERMARKET, SECTOR
// Each family has: availability, components, score, reasoning
// ============================================================================
...
export function buildEvidenceFamilies(components) {
  const families = {};

  // MARKET_STRUCTURE: trend + momentum + volatility (from indices)
  families[EVIDENCE_FAMILIES.MARKET_STRUCTURE] = {
    family: EVIDENCE_FAMILIES.MARKET_STRUCTURE,
    label: 'Market Structure',
    availability: 'AVAILABLE',
    components: [],
    score: null,
    reasons: [],
    dataSources: ['app/api/indices (Yahoo Finance Chart)'],
  };

  const ms = families[EVIDENCE_FAMILIES.MARKET_STRUCTURE];
  if (components.trend?.score != null) {
    ms.components.push('trend');
    ms.reasons.push(...components.trend.reasons);
  }
  if (components.momentum?.score != null) {
    ms.components.push('momentum');
    ms.reasons.push(...components.momentum.reasons);
  }
  if (components.volatility?.score != null) {
    ms.components.push('volatility');
    ms.reasons.push(...components.volatility.reasons);
  }
  if (ms.components.length > 0) {
    const scores = ms.components.map(k => components[k]?.score).filter(s => s != null);
    ms.score = scores.length > 0 ? Math.round(scores.reduce((a, b) => a + b, 0) / scores.length) : null;
  } else {
    ms.availability = 'UNAVAILABLE';
    ms.reasons.push('No market structure data available');
  }

  // BREADTH: breadth + volume + priceBreadth + rsiDistribution (from universe)
  families[EVIDENCE_FAMILIES.BREADTH] = {
    family: EVIDENCE_FAMILIES.BREADTH,
    label: 'Market Breadth',
    availability: 'AVAILABLE',
    components: [],
    score: null,
    reasons: [],
    dataSources: ['app/api/stocks (universe)'],
  };

  const br = families[EVIDENCE_FAMILIES.BREADTH];
  if (components.breadth?.score != null) {
    br.components.push('breadth');
    br.reasons.push(...components.breadth.reasons);
  }
  if (components.volume?.score != null) {
    br.components.push('volume');
    br.reasons.push(...components.volume.reasons);
  }
  if (components.priceBreadth?.score != null) {
    br.components.push('priceBreadth');
    br.reasons.push(...components.priceBreadth.reasons);
  }
  if (components.rsiDistribution?.score != null) {
    br.components.push('rsiDistribution');
    br.reasons.push(...components.rsiDistribution.reasons);
  }
  if (br.components.length > 0) {
    const scores = br.components.map(k => components[k]?.score).filter(s => s != null);
    br.score = scores.length > 0 ? Math.round(scores.reduce((a, b) => a + b, 0) / scores.length) : null;
  } else {
    br.availability = 'UNAVAILABLE';
    br.reasons.push('No breadth data available');
  }

  // INTERMARKET: bonds, commodities, currencies - NO DATA AVAILABLE
  families[EVIDENCE_FAMILIES.INTERMARKET] = {
    family: EVIDENCE_FAMILIES.INTERMARKET,
    label: 'Intermarket (Bonds, Commodities, Currencies)',
    availability: 'UNAVAILABLE',
    components: [],
    score: null,
    reasons: ['Intermarket data not available - no bond, commodity, or currency data in current data sources'],
    dataSources: [],
  };

  // SECTOR: sector-level rotation - NO DATA AVAILABLE
  families[EVIDENCE_FAMILIES.SECTOR] = {
    family: EVIDENCE_FAMILIES.SECTOR,
    label: 'Sector Rotation',
    availability: 'UNAVAILABLE',
    components: [],
    score: null,
    reasons: ['Sector data not available - no sector classification in current universe data'],
    dataSources: [],
  };
...
export function buildRegimeWarnings(components, label) {
  ...
  if (flags.includes('BROAD_NEGATIVE')) warnings.push('BROAD_NEGATIVE_BREADTH');
  if (flags.includes('SEVERE_BREADTH_DECLINE')) warnings.push('SEVERE_BREADTH_DECLINE');
  if (flags.includes('WIDE_OVERBOUGHT')) warnings.push('WIDE_OVERBOUGHT');
  if (flags.includes('WIDE_OVERSOLD')) warnings.push('WIDE_OVERSOLD');
...
export function buildRegimeRisks(components, label) {
  ...
  if (flags.includes('BROAD_NEGATIVE') || flags.includes('SEVERE_BREADTH_DECLINE')) {
    risks.push({ label: 'BREADTH_DECLINE', severity: 'MEDIUM', description: 'Majority of stocks are declining - broad market weakness' });
  }
  if (flags.includes('WIDE_OVERBOUGHT')) {
    risks.push({ label: 'OVERBOUGHT_CONCENTRATION', severity: 'LOW', description: 'High percentage of stocks in overbought territory - potential pullback risk' });
  }
  if (flags.includes('WIDE_OVERSOLD')) {
    risks.push({ label: 'OVERSOLD_CONCENTRATION', severity: 'LOW', description: 'High percentage of stocks in oversold territory - potential bounce opportunity' });
  }
...
function collectFlags(components) {
  const flags = [];
  for (const key of ['trend', 'momentum', 'breadth', 'volume', 'volatility', 'priceBreadth', 'rsiDistribution']) {
...
export function buildSupportingEvidence(components, componentWeights) {
  ...
  const labels = {
    ...
    breadth: 'Market breadth (setupScore)',
    volume: 'Volume confirmation',
    volatility: 'Volatility / risk',
    priceBreadth: 'Price breadth (% positive change)',
...
export function buildProvenance(components) {
  ...
  const known = {
    ...
    priceBreadth: 'app/api/stocks (universe changePercent)',
    rsiDistribution: 'app/api/stocks (universe rsi)',
    data: 'C10 computed',
  };
...
export function buildMarketRegime(indices, universe) {
  ...
  const priceBreadth = scorePriceBreadth(uniArr);
  const rsiDistribution = scoreRSIDistribution(uniArr);
  ...
  // ---- Evidence families ----
  const evidenceFamilies = buildEvidenceFamilies(componentMap);
  ...
  if (uniArr.length === 0) limitations.push('Universe unavailable - breadth, volume, priceBreadth, and rsiDistribution excluded');
  if (priceBreadth.score == null && uniArr.length > 0) limitations.push('Price breadth unavailable - no changePercent data in universe');
  if (rsiDistribution.score == null && uniArr.length > 0) limitations.push('RSI distribution unavailable - no RSI data in universe');
  limitations.push('Intermarket data not available - no bond, commodity, or currency data');
  limitations.push('Sector data not available - no sector classification in universe');
  ...
  return {
    ...
    disclaimer: 'Market Regime (C10) is context only - not a buy/sell signal. ' +
      ...
      'Intermarket and sector data are unavailable - marked as UNAVAILABLE, not fabricated.',
  };
}
