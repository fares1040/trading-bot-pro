...
function buildOpportunityWarnings(sources, score, quality, conflictingSignals) {
  ...
  if (conflictingSignals && conflictingSignals.length > 0) {
    ...
    for (const conflict of conflictingSignals.slice(0, 3)) {
      warnings.push(`CONFLICT: ${conflict.description || conflict.type}`);
    }
  }
...
// ============================================================================
// EXPECTED TIMEFRAME AGGREGATION (P2-2)
// ============================================================================

/**
 * Aggregate expectedTimeframe from B1-B6 intelligence sources.
 *
 * Priority order (highest-quality / most reliable horizon first):
 *   1. B4 Swing Intelligence  → '1–3 Months' (TOP/STRONG) or '1–2 Weeks' (WATCH)
 *   2. B5 Early Explosion      → 'SHORT_TERM' (EXTREME/HIGH)
 *   3. B2 Options Intelligence → 'OPTIONS' (always, never convert DTE)
 *   4. B1/B3/B6               → 'UNKNOWN' (no reliable horizon)
 *
 * Does NOT fabricate, guess, or convert DTE to holding period.
 * Returns 'UNKNOWN' when no source provides a usable value.
 */
function aggregateExpectedTimeframe(sources) {
  const candidates = [];

  // B4 — Swing Intelligence (most reliable horizon)
  if (sources.swingIntelligence && typeof sources.swingIntelligence === 'object') {
    const v = sources.swingIntelligence.expectedTimeframe;
    if (v != null && v !== 'UNKNOWN') {
      candidates.push({ source: 'swingIntelligence', value: v, priority: 1 });
    }
  }

  // B5 — Early Explosion
  if (sources.earlyExplosion && typeof sources.earlyExplosion === 'object') {
    const v = sources.earlyExplosion.expectedTimeframe;
    if (v != null && v !== 'UNKNOWN') {
      candidates.push({ source: 'earlyExplosion', value: v, priority: 2 });
    }
  }

  // B2 — Options Intelligence (always 'OPTIONS', never convert DTE)
  if (sources.optionsIntelligence && typeof sources.optionsIntelligence === 'object') {
    const v = sources.optionsIntelligence.expectedTimeframe;
    if (v != null && v !== 'UNKNOWN') {
      candidates.push({ source: 'optionsIntelligence', value: v, priority: 3 });
    }
  }

  if (candidates.length === 0) return 'UNKNOWN';

  // Sort by priority ascending; first candidate wins.
...
function buildOpportunityRanking(symbol, sources, options = {}) {
  ...
  const technicalEvidence = options.technicalEvidence || null;
  ...
  // --- Technical Evidence Integration (shadow mode) ---
  // Technical Evidence provides family-level interpretation without changing
  // the current C7 score. Shadow comparison enables future safe migration.
  let technicalEvidenceFamilies = null;
  let technicalEvidenceShadow = null;

  if (technicalEvidence && typeof technicalEvidence === 'object') {
    technicalEvidenceFamilies = technicalEvidence.families || null;

    // Shadow interpretation: derive direction/strength from evidence families
    // without modifying the existing opportunityScore
    if (technicalEvidenceFamilies) {
      const familyDirs = [];
      for (const [familyName, family] of Object.entries(technicalEvidenceFamilies)) {
        if (family && family.direction && family.direction !== 'UNAVAILABLE') {
          familyDirs.push({ family: familyName, direction: family.direction, strength: family.strength });
        }
      }

      const bullishFamilies = familyDirs.filter(f => f.direction === 'BULLISH').length;
      const bearishFamilies = familyDirs.filter(f => f.direction === 'BEARISH').length;
      const totalFamilies = familyDirs.length;

      let shadowDirection = 'NEUTRAL';
      if (bullishFamilies > bearishFamilies) shadowDirection = 'BULLISH';
      else if (bearishFamilies > bullishFamilies) shadowDirection = 'BEARISH';

      // Conflict detection: structure vs trend vs momentum
      const structureDir = technicalEvidenceFamilies.structure?.direction;
      const trendDir = technicalEvidenceFamilies.trend?.direction;
      const momentumDir = technicalEvidenceFamilies.momentum?.direction;
      const structuralConflicts = [];
      if (structureDir && trendDir && structureDir !== 'UNAVAILABLE' && trendDir !== 'UNAVAILABLE' && structureDir !== trendDir) {
        structuralConflicts.push(`STRUCTURE(${structureDir}) vs TREND(${trendDir})`);
      }
      if (trendDir && momentumDir && trendDir !== 'UNAVAILABLE' && momentumDir !== 'UNAVAILABLE' && trendDir !== momentumDir) {
        structuralConflicts.push(`TREND(${trendDir}) vs MOMENTUM(${momentumDir})`);
      }

      technicalEvidenceShadow = {
        direction: shadowDirection,
        familyCount: totalFamilies,
        bullishFamilies,
        bearishFamilies,
        conflicts: structuralConflicts,
        alignment: structuralConflicts.length === 0 && totalFamilies >= 3 ? 'ALIGNED'
          : structuralConflicts.length > 0 ? 'CONFLICTING'
          : 'PARTIAL',
      };
    }
  }
  ...
  return {
    ...
    technicalEvidence: technicalEvidenceFamilies,
    technicalEvidenceShadow,

    expectedTimeframe: aggregateExpectedTimeframe({
      pennyIntelligence: b1,
      optionsIntelligence: b2,
      institutionalRadar: b3,
      swingIntelligence: b4,
    ...
    disclaimer:
      ...
      'Missing intelligence sources remain null — never fabricated. ' +
      'Scores are evidence-weighted aggregations of B1–B6 outputs. ' +
      'Technical Evidence families are provided for structural context — not used in scoring yet. ' +
