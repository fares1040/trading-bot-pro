...
export function normalizeOptionContract(x, side, underlying, expiry) {
  ...
  const volume = n(x.volume);
  const openInterest = n(x.openInterest);
  ...
  const now = Date.now();
  const expiryMs = expiry != null ? expiry * 1000 : null;
  const daysToExpiration =
    expiryMs != null ? Math.max(0, Math.round((expiryMs - now) / (1000 * 60 * 60 * 24))) : null;
  const distancePct =
    underlying && strike ? Math.abs((strike - underlying) / underlying) * 100 : null;

  // Liquidity score (0-100): volume + open interest + tight spread.
  // When volume/OI are missing, use neutral base (25) instead of 0 to avoid penalizing
  // incomplete data. Missing data is NOT treated as zero volume.
  const liquidityScore =
    (volume != null ? Math.min(100, volume * 1.2) : 25) * 0.45 +
    (openInterest != null ? Math.min(100, openInterest / 5) : 25) * 0.35 +
...
async function fetchYahooOptionsJson(symbol) {
  ...
  try {
    ...
    recordProviderFailure('yahoo', error, 'options-data', symbol, false);
    return { ok: false, json: null, error: `yahoo-options-request-failed:${error?.message || 'unknown'}` };
  }

  if (!response.ok) {
    if (response.status === 401 || response.status === 403) {
      recordProviderFailure('yahoo', new Error('Yahoo Options AUTH_FAILURE'), 'options-data', symbol, false);
      return { ok: false, json: null, error: 'yahoo-options-auth-required' };
    }
    if (response.status === 404) {
      recordProviderFailure('yahoo', new Error(`Yahoo Options ${response.status}`), 'options-data', symbol, false);
      return { ok: false, json: null, error: 'yahoo-options-404' };
    }
    if (response.status === 429) {
      recordProviderFailure('yahoo', new Error('Yahoo Options rate limit'), 'options-data', symbol, false);
      return { ok: false, json: null, error: 'yahoo-options-429' };
    }
    recordProviderFailure('yahoo', new Error(`Yahoo Options ${response.status}`), 'options-data', symbol, false);
  ...
  if (!text || !text.trim()) {
    recordProviderFailure('yahoo', new Error('Yahoo Options empty response'), 'options-data', symbol, false);
  ...
  try {
    ...
    recordProviderFailure('yahoo', new Error('Yahoo Options invalid JSON'), 'options-data', symbol, false);
