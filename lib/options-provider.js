...
async function fetchYahooOptionsJson(symbol) {
  ...
  if (!response.ok) {
    // Authoritative non-OK handler. Specific status codes get named reasons so
    // callers can distinguish auth-gated endpoints, missing symbols, and rate
    // limiting from generic upstream failures. Never fabricate data here.
