...
function directionFrom(value, threshold = 0.05) {
...
/**
 * Build a deterministic live-market pulse from the current snapshot and a
 * short sequence of prior snapshots. No market data is created here.
 */
export function buildLiveMarketPulse(current, history = []) {
  ...
  const priorPriceChange = previous && previousPrevious
    ? pct(finite(previous.price), finite(previousPrevious.price))
    : null;
  const priceAccelerationPercent = priceChangePercent == null || priorPriceChange == null
    ? null
    : priceChangePercent - priorPriceChange;

  const priorVolumeChange = previous && previousPrevious
    ? pct(finite(previous.volume), finite(previousPrevious.volume))
    : null;
  const volumeAccelerationPercent = volumeChangePercent == null || priorVolumeChange == null
    ? null
    : volumeChangePercent - priorVolumeChange;

  const spread = current?.spread ?? null;
  const bid = finite(current?.bid);
  const ask = finite(current?.ask);
  const derivedSpread = bid != null && ask != null && bid > 0
    ? ((ask - bid) / bid) * 100
    : finite(spread);
  ...
  const pressureScore = Math.round(
    momentum * 0.35 + acceleration * 0.25 + volumePressure * 0.25 + vwapBias * 0.15
  );

  const direction = directionFrom(
    (priceChangePercent || 0) * 0.5 + (priceAccelerationPercent || 0) * 0.5
  );

  const accelerationState =
    priceAccelerationPercent == null || volumeAccelerationPercent == null
      ? 'INSUFFICIENT_DATA'
      : priceAccelerationPercent > 0.15 && volumeAccelerationPercent > 5
        ? 'ACCELERATING'
        : priceAccelerationPercent < -0.15 && volumeAccelerationPercent > 5
          ? 'DECELERATING_DOWN'
          : 'STABLE';
  ...
  return {
    ...
    mode: PULSE_MODE,
    symbol: String(current?.symbol || '').toUpperCase(),
    timestamp: current?.timestamp || null,
    fetchedAt: current?.fetchedAt || null,
    freshness: current?.freshness || null,
    direction,
    price,
    priceChangePercent: priceChangePercent == null ? null : Number(priceChangePercent.toFixed(4)),
    priceAccelerationPercent: priceAccelerationPercent == null ? null : Number(priceAccelerationPercent.toFixed(4)),
    volume,
    volumeChangePercent: volumeChangePercent == null ? null : Number(volumeChangePercent.toFixed(4)),
    volumeAccelerationPercent: volumeAccelerationPercent == null ? null : Number(volumeAccelerationPercent.toFixed(4)),
    vwap,
    vwapDistancePercent: vwapDistancePercent == null ? null : Number(vwapDistancePercent.toFixed(4)),
    spreadPercent: derivedSpread == null ? null : Number(derivedSpread.toFixed(4)),
    momentumScore: Math.round(momentum),
    accelerationScore: Math.round(acceleration),
    volumePressureScore: Math.round(volumePressure),
    vwapBiasScore: Math.round(vwapBias),
    pressureScore,
    accelerationState,
    evidence,
    dataQuality: current?.dataQuality || 'UNKNOWN',
    source: current?.source || null,
    disclaimer: 'Pulse metrics are derived from supplied market snapshots; they are not independent real-time order-flow or institutional-flow data.',
  };
}
