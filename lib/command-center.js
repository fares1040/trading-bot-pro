...
export function buildCommandCenter(regimeData, opportunityData, tradePlanData, aiExplanationData, options = {}) {
  const marketRegime = buildMarketRegimeSection(regimeData);
  const topOpportunities = buildTopOpportunitiesSection(opportunityData, options.limit || 5);
  const tradePlans = buildTradePlansSection(tradePlanData, options.limit || 5);
  ...
  const totalResults = topOpportunities.count || 0;
  const hasNext = page * limit < totalResults;
