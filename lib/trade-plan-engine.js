...
// ============================================================================
// TECHNICAL EVIDENCE EXTRACTION (Step 6)
// Extracts structural levels from Technical Evidence for trade plan generation.
// Provides fallback levels when B4/B5 zones are unavailable.
// Never fabricates levels — only uses objective data from Technical Evidence.
// ============================================================================
function extractTE_SRLevels(technicalEvidence) {
  if (!technicalEvidence || typeof technicalEvidence !== "object") return null;
  const families = technicalEvidence.families;
  if (!families || typeof families !== "object") return null;

  const structure = families.structure;
  ...
  // Extract support/resistance from BOS/CHoCH levels
  for (const obs of observations) {
    if (obs.type === "BOS" && obs.level != null) {
      if (obs.direction === "BULLISH" && supportLevel == null) {
        supportLevel = n(obs.level);
      } else if (obs.direction === "BEARISH" && resistanceLevel == null) {
        resistanceLevel = n(obs.level);
      }
    }
    if (obs.type === "CHoCH" && obs.level != null) {
      if (obs.direction === "BULLISH" && supportLevel == null) {
        supportLevel = n(obs.level);
      } else if (obs.direction === "BEARISH" && resistanceLevel == null) {
        resistanceLevel = n(obs.level);
  ...
  if (supportLevel == null && resistanceLevel == null) return null;
  return { supportLevel, resistanceLevel };
}

function extractTE_ATR(technicalEvidence) {
  if (!technicalEvidence || typeof technicalEvidence !== "object") return null;
  const families = technicalEvidence.families;
  if (!families || typeof families !== "object") return null;

  const volatility = families.volatility;
  if (!volatility || typeof volatility !== "object") return null;

  const observations = Array.isArray(volatility.observations) ? volatility.observations : [];
  for (const obs of observations) {
    if (obs.type === "HIGH_ATR" || obs.type === "LOW_ATR") {
      if (obs.value != null) return n(obs.value);
    }
  }
  return null;
}

function extractTEStructure(technicalEvidence) {
  if (!technicalEvidence || typeof technicalEvidence !== "object") return null;
  const families = technicalEvidence.families;
  if (!families || typeof families !== "object") return null;
  ...
  for (const obs of observations) {
    if (obs.type === "BOS" && obs.level != null) {
      bosLevel = n(obs.level);
      structureDirection = obs.direction;
    }
    if (obs.type === "CHoCH" && obs.level != null) {
      chochLevel = n(obs.level);
      structureDirection = obs.direction;
    }
  }

  return { bosLevel, chochLevel, structureDirection };
}

// ============================================================================
// ZONE / LEVEL EXTRACTION (prefer existing B4/B5 zones, never fabricated)
// ============================================================================
function firstZone(b4, b5, technicalEvidence = null) {
  ...
  // Step 6: Fall back to Technical Evidence structural levels when B4/B5 unavailable.
  const teSR = extractTE_SRLevels(technicalEvidence);
  const teStructure = extractTEStructure(technicalEvidence);
  if (teSR || teStructure) {
    return {
      entryZone: null,
      entryPrice: null,
      target1: teSR ? teSR.resistanceLevel : null,
      target2: null,
      invalidation: teStructure ? teStructure.chochLevel : null,
      stopLoss: teStructure ? teStructure.bosLevel : null,
      riskPercent: null,
      rewardPercent: null,
      riskReward: null,
    };
...
// ============================================================================
function resolveDirection(sources, technicalEvidence = null) {
  ...
  // Step 6: Fall back to Technical Evidence direction when upstream unavailable.
  if (technicalEvidence && technicalEvidence.overallDirection) {
    const teDir = String(technicalEvidence.overallDirection).toUpperCase();
    if (teDir === "BULLISH") return DIRECTION.LONG;
    if (teDir === "BEARISH") return DIRECTION.SHORT;
    if (teDir === "NEUTRAL") return DIRECTION.NEUTRAL;
  }
...
function derivePlanLevels(zones, price, direction, technicalEvidence = null) {
  ...
  // Step 6: Use ATR to fill missing stop/target when B4/B5 zones incomplete
  const atr = extractTE_ATR(technicalEvidence);
  if (atr != null && price != null) {
    // ATR-based stop: 1.5x ATR from entry
    if (result.stopLoss == null) {
      const atrStop = direction === DIRECTION.SHORT
        ? round(price + atr * 1.5, 2)
        : round(price - atr * 1.5, 2);
      result.stopLoss = atrStop;
    }
    // ATR-based target: 2x ATR from entry
    if (result.target1 == null) {
      const atrTarget = direction === DIRECTION.SHORT
        ? round(price - atr * 2, 2)
        : round(price + atr * 2, 2);
      result.target1 = atrTarget;
    }
  }
...
function buildTradePlan(symbol, sources, opts = {}) {
  ...
  const technicalEvidence = opts.technicalEvidence || null;

  const availableCount = Object.values(sources).filter((v) => v != null).length;
  const hasTE = technicalEvidence && typeof technicalEvidence === "object";
  if (availableCount === 0 && !hasTE) {
    return {
      ...defaultResult,
      symbol,
      reasons: ["No intelligence sources or technical evidence provided for trade plan"],
    };
  }

  // Direction (evidence-based, never coerced)
  const direction = resolveDirection(sources, technicalEvidence);
  ...
  // Zones / levels (prefer B4/B5, fallback to Technical Evidence S/R)
  const zones = firstZone(b4, b5, technicalEvidence);
  const price = extractPrice(c7, b4, b5, b1);
  const levels = derivePlanLevels(zones, price, direction, technicalEvidence);
  ...
  const invalidationItems = horizonEvidence && Array.isArray(horizonEvidence.invalidation) ? horizonEvidence.invalidation.slice(0, 4) : [];
  const watchItems = horizonEvidence && Array.isArray(horizonEvidence.watchItems) ? horizonEvidence.watchItems.slice(0, 4) : [];

  // Step 6: Extract Technical Evidence structural context for output
  const teSR = extractTE_SRLevels(technicalEvidence);
  const teATR = extractTE_ATR(technicalEvidence);
  const teStructure = extractTEStructure(technicalEvidence);
  const teLevelSource = technicalEvidence ? {
    supportLevel: teSR ? teSR.supportLevel : null,
    resistanceLevel: teSR ? teSR.resistanceLevel : null,
    atr: teATR,
    bosLevel: teStructure ? teStructure.bosLevel : null,
    chochLevel: teStructure ? teStructure.chochLevel : null,
    direction: technicalEvidence.overallDirection || null,
    usedForStop: levels.stopLoss != null && teATR != null && (!b4 || !b4.stopLoss),
    usedForTarget: levels.target1 != null && teSR != null && (!b4 || !b4.target1),
  } : null;
  ...
  return {
    ...
    technicalEvidence: teLevelSource,

    provenance,
    limitations: "C8 composes C7 + B1-B6. Levels come from upstream zone engines (B4/B5) with Technical Evidence (S/R, ATR, structure) as structural fallback. Missing components are excluded and weights renormalized.",
    disclaimer: "Trade Plan (C8) is conditional only - not a buy/sell recommendation. Levels without trustworthy data remain unavailable. Missing levels remain null - never fabricated.",
    reasons,
  };
}
