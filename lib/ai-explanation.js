...
function explainCatalyst(b6) {
  ...
  let riskOut = null;
  if (risk && risk !== "LOW" && risk !== "UNKNOWN") riskOut = "Catalyst risk: " + risk;
  ...
  if (sourceHasData(b6)) {
    ...
    if ((score != null && score >= 60) || types.length > 0) {
      factors.push({
        ...
        strength: score,
        explanation: "Verified catalyst evidence" +
          (types.length > 0 ? " (" + types.join(", ") + ")" : "") + " supports current notability.",
        evidence: types.length > 0 ? ["Catalyst types: " + types.join(", ")] : [],
        risk: (b6.catalystRisk && b6.catalystRisk !== "LOW") ? "Catalyst risk: " + b6.catalystRisk : null,
      });
    }
  }
