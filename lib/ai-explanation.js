...
// ---- Technical Evidence (Step 7) -------------------------------------------
// Explains Technical Evidence by family. Each family is ONE evidence block,
// not independent signals. Anti-double-counting: observations within a family
// are grouped, not counted separately.
// ============================================================================
function explainTechnicalEvidenceFamily(familyName, family) {
  if (!family || typeof family !== "object") return null;
  if (family.direction === "UNAVAILABLE" || family.direction == null) return null;

  const observations = Array.isArray(family.observations) ? family.observations : [];
  if (observations.length === 0 && family.strength == null) return null;
  ...
  const evidence = [];
  if (strength != null) evidence.push(familyName + " strength " + strength + "/100");
  if (confidence != null) evidence.push("Confidence " + confidence + "/100");
  for (const obs of observations.slice(0, 4)) {
    if (obs.description) evidence.push(obs.description);
  }
  ...
  switch (familyName) {
    case "STRUCTURE": {
      const hh = observations.filter(o => o.type === "HH").length;
      const hl = observations.filter(o => o.type === "HL").length;
      const lh = observations.filter(o => o.type === "LH").length;
      const ll = observations.filter(o => o.type === "LL").length;
      const bos = observations.find(o => o.type === "BOS");
      const choch = observations.find(o => o.type === "CHoCH");
      const mtf = observations.find(o => o.type === "MTF_ALIGNMENT" || o.type === "MTF_CONFLICT");

      const parts = [];
      if (hh > 0 || hl > 0) parts.push("higher highs/lows structure");
      if (lh > 0 || ll > 0) parts.push("lower highs/lows structure");
      if (bos) parts.push("bullish break of structure" + (bos.level != null ? " at " + bos.level : ""));
      if (choch) parts.push("change of character" + (choch.level != null ? " at " + choch.level : "") + " — potential structural transition");
      if (mtf && mtf.type === "MTF_ALIGNMENT") parts.push("multi-timeframe alignment");
      if (mtf && mtf.type === "MTF_CONFLICT") parts.push("multi-timeframe structural conflict");

      explanation = "Price structure is " + direction.toLowerCase() +
        (parts.length > 0 ? ", with " + parts.join(", ") : "") + ".";
      break;
    }
    case "TREND": {
      const priceObs = observations.find(o => o.type === "PRICE_ABOVE_MA" || o.type === "PRICE_BELOW_MA");
      const slopeObs = observations.find(o => o.type === "MA_SLOPE_UP" || o.type === "MA_SLOPE_DOWN");

      const parts = [];
      if (priceObs) parts.push(priceObs.description);
      if (slopeObs) parts.push(slopeObs.description);

      explanation = "Trend context is " + direction.toLowerCase() +
        (parts.length > 0 ? " — " + parts.join(", ") : "") + ".";
      break;
    }
    case "MOMENTUM": {
      const rsiObs = observations.find(o => o.type === "RSI_OVERBOUGHT" || o.type === "RSI_OVERSOLD" || o.type === "RSI_NEUTRAL");
      const parts = [];
      if (rsiObs) parts.push(rsiObs.description);

      explanation = "Momentum is " + direction.toLowerCase() +
        (parts.length > 0 ? " — " + parts.join(", ") : "") + ".";
      break;
    }
    case "VOLUME": {
      const rvolObs = observations.find(o => o.type === "HIGH_RVOL" || o.type === "LOW_RVOL");
      const breakoutObs = observations.find(o => o.type === "BREAKOUT_VOLUME");
      const parts = [];
      if (rvolObs) parts.push(rvolObs.description);
      if (breakoutObs) parts.push("volume confirming near-resistance proximity");

      explanation = "Volume context is " + direction.toLowerCase() +
        (parts.length > 0 ? " — " + parts.join(", ") : "") + ".";
      break;
    }
    case "VOLATILITY": {
      const squeezeObs = observations.find(o => o.type === "BOLLINGER_SQUEEZE");
      const atrObs = observations.find(o => o.type === "HIGH_ATR" || o.type === "LOW_ATR");
      const parts = [];
      if (squeezeObs) parts.push("Bollinger squeeze detected — volatility compression");
      if (atrObs) parts.push(atrObs.description);

      explanation = "Volatility context" +
        (parts.length > 0 ? ": " + parts.join(", ") : " is neutral") + ".";
      break;
    }
    case "PATTERN": {
      const candleObs = observations.filter(o => o.type === "CANDLESTICK");
      const classicalObs = observations.filter(o => o.type !== "CANDLESTICK");

      const parts = [];
      if (candleObs.length > 0) parts.push(candleObs.length + " candlestick pattern(s)");
      if (classicalObs.length > 0) parts.push(classicalObs.length + " structural pattern(s)");

      explanation = "Pattern context is " + direction.toLowerCase() +
        (parts.length > 0 ? " — " + parts.join(", ") : "") + ".";
      break;
    }
    default:
      explanation = familyName + " evidence is " + direction.toLowerCase() + ".";
  }

  let risk = null;
  if (direction === "BEARISH") risk = familyName + " evidence is bearish";
  if (observations.some(o => o.type === "MTF_CONFLICT")) risk = "Multi-timeframe structural conflict detected";

  return block("TE " + familyName, signal, explanation, evidence, risk);
}

function explainTechnicalEvidenceFull(te) {
  if (!te || typeof te !== "object") return null;
  const families = te.families;
  if (!families || typeof families !== "object") return null;

  const familyBlocks = [];
  for (const [name, family] of Object.entries(families)) {
    const b = explainTechnicalEvidenceFamily(name, family);
    if (b) familyBlocks.push(b);
  }

  if (familyBlocks.length === 0) return null;

  const overallDir = te.overallDirection || "UNAVAILABLE";
  const availableCount = te.availableFamilies || 0;

  const evidence = familyBlocks.flatMap(b => b.evidence || []);

  let explanation = "Technical evidence across " + availableCount + " families: " +
    familyBlocks.map(b => (b.factor || "").replace("TE ", "") + " " + (b.signal || "").toLowerCase()).join(", ") + ".";

  return {
    explanation,
    evidence,
    families: familyBlocks,
...
// ============================================================================
function buildWhyNow(sources, explanations, te) {
  const factors = [];
  ...
  // Step 7: Technical Evidence factors
  if (te && typeof te === "object" && te.families) {
    const dir = te.overallDirection;
    if (dir && dir !== "UNAVAILABLE") {
      // Structure family
      const struct = te.families.structure;
      if (struct && struct.direction !== "UNAVAILABLE" && struct.observations && struct.observations.length > 0) {
        const bos = struct.observations.find(o => o.type === "BOS");
        const choch = struct.observations.find(o => o.type === "CHoCH");
        const hh = struct.observations.filter(o => o.type === "HH").length;
        const hl = struct.observations.filter(o => o.type === "HL").length;

        const parts = [];
        if (bos) parts.push("bullish BOS" + (bos.level != null ? " at " + bos.level : ""));
        if (choch) parts.push("CHoCH" + (choch.level != null ? " at " + choch.level : ""));
        if (hh > 0 || hl > 0) parts.push("higher highs/lows");

        if (parts.length > 0) {
          factors.push({
            factor: "TE Structure",
            signal: struct.direction,
            strength: struct.strength,
            explanation: "Price structure is " + struct.direction.toLowerCase() + " — " + parts.join(", ") + ".",
            evidence: struct.observations.slice(0, 3).map(o => o.description || o.type),
            risk: choch ? "CHoCH detected — potential structural transition" : null,
          });
        }
      }

      // Trend family
      const trend = te.families.trend;
      if (trend && trend.direction !== "UNAVAILABLE" && trend.observations && trend.observations.length > 0) {
        factors.push({
          factor: "TE Trend",
          signal: trend.direction,
          strength: trend.strength,
          explanation: "Trend context is " + trend.direction.toLowerCase() + ".",
          evidence: trend.observations.slice(0, 3).map(o => o.description || o.type),
          risk: null,
        });
      }

      // Momentum family
      const momentum = te.families.momentum;
      if (momentum && momentum.direction !== "UNAVAILABLE" && momentum.observations && momentum.observations.length > 0) {
        factors.push({
          factor: "TE Momentum",
          signal: momentum.direction,
          strength: momentum.strength,
          explanation: "Momentum is " + momentum.direction.toLowerCase() + ".",
          evidence: momentum.observations.slice(0, 3).map(o => o.description || o.type),
          risk: null,
        });
      }

      // Volume family
      const volume = te.families.volume;
      if (volume && volume.direction !== "UNAVAILABLE" && volume.observations && volume.observations.length > 0) {
        factors.push({
          factor: "TE Volume",
          signal: volume.direction,
          strength: volume.strength,
          explanation: "Volume context is " + volume.direction.toLowerCase() + ".",
          evidence: volume.observations.slice(0, 3).map(o => o.description || o.type),
          risk: null,
        });
      }

      // Volatility family
      const volatility = te.families.volatility;
      if (volatility && volatility.observations && volatility.observations.length > 0) {
        factors.push({
          factor: "TE Volatility",
          signal: volatility.direction || "NEUTRAL",
          strength: volatility.strength,
          explanation: "Volatility context: " + volatility.observations.slice(0, 2).map(o => o.description || o.type).join(", ") + ".",
          evidence: volatility.observations.slice(0, 3).map(o => o.description || o.type),
          risk: null,
        });
      }

      // Pattern family
      const pattern = te.families.pattern;
      if (pattern && pattern.direction !== "UNAVAILABLE" && pattern.observations && pattern.observations.length > 0) {
        factors.push({
          factor: "TE Pattern",
          signal: pattern.direction,
          strength: pattern.strength,
          explanation: "Pattern context is " + pattern.direction.toLowerCase() + " — " + pattern.observations.length + " pattern(s) detected.",
          evidence: pattern.observations.slice(0, 3).map(o => o.description || o.type),
          risk: null,
        });
      }
    }
...
// ============================================================================
function detectConflicts(sources, te) {
  const conflicts = [];
  const signals = [];
  ...
  // TE family-level conflicts (Step 7)
  if (te && typeof te === "object" && te.families) {
    const structDir = te.families.structure && te.families.structure.direction;
    const trendDir = te.families.trend && te.families.trend.direction;
    const momDir = te.families.momentum && te.families.momentum.direction;

    if (structDir && trendDir && structDir !== "UNAVAILABLE" && trendDir !== "UNAVAILABLE" && structDir !== trendDir) {
      conflicts.push({
        type: "TE_FAMILY_CONFLICT",
        severity: "MEDIUM",
        description: "STRUCTURE(" + structDir + ") vs TREND(" + trendDir + ")",
        sources: ["technicalEvidence"],
      });
    }
    if (trendDir && momDir && trendDir !== "UNAVAILABLE" && momDir !== "UNAVAILABLE" && trendDir !== momDir) {
      conflicts.push({
        type: "TE_FAMILY_CONFLICT",
        severity: "MEDIUM",
        description: "TREND(" + trendDir + ") vs MOMENTUM(" + momDir + ")",
        sources: ["technicalEvidence"],
      });
    }

    const mtfConflict = te.families.structure && te.families.structure.observations &&
      te.families.structure.observations.find(o => o.type === "MTF_CONFLICT");
    if (mtfConflict) {
      conflicts.push({
        type: "TE_MTF_CONFLICT",
        severity: "MEDIUM",
        description: "Multi-timeframe structural conflict detected",
        sources: ["technicalEvidence"],
      });
    }
  }
...
// ============================================================================
// WHAT CONFIRMS (Step 7)
// Lists evidence that supports the current thesis. Only factual, traceable
// claims. Never fabricates confirmation.
// ============================================================================
function buildConfirms(sources, te, levels) {
  const confirms = [];

  // TE family confirmations
  if (te && typeof te === "object" && te.families) {
    const dir = te.overallDirection;
    for (const [name, family] of Object.entries(te.families)) {
      if (!family || family.direction === "UNAVAILABLE" || family.direction == null) continue;
      if (family.direction === dir || (dir === "NEUTRAL" && family.direction === "NEUTRAL")) {
        confirms.push({
          source: "TE " + name,
          description: name + " evidence is " + family.direction.toLowerCase(),
        });
      }
    }
  }
  ...
  // C7 confirmation
  const c7 = sources.opportunityRanking;
  if (sourceHasData(c7) && c7.opportunityScore != null && c7.opportunityScore >= 70) {
    confirms.push({
      source: "C7 Opportunity Ranking",
      description: "Cross-intelligence score " + c7.opportunityScore + "/100 supports the thesis",
    });
  }

  // C8 confirmation
  const tp = sources.tradePlan;
  if (sourceHasData(tp) && tp.planScore != null && tp.planScore >= 70) {
    confirms.push({
      source: "C8 Trade Plan",
      description: "Conditional plan available with score " + tp.planScore + "/100",
    });
  }

  // B4/B5 setup confirmation
  for (const key of ["swingIntelligence", "earlyExplosion"]) {
    const src = sources[key];
    if (sourceHasData(src) && Array.isArray(src.setupClassification) && src.setupClassification.length > 0) {
      confirms.push({
        source: key === "swingIntelligence" ? "B4 Swing" : "B5 Explosion",
        description: src.setupClassification[0].type + " setup detected",
      });
    }
  }
...
// ============================================================================
// WHAT INVALIDATES (Step 7)
// Lists evidence that would invalidate the current thesis. Uses actual
// C8/structure levels when available. Never fabricates price levels.
// ============================================================================
function buildInvalidates(sources, te, levels) {
  const invalidates = [];

  // TE structural invalidation
  if (te && typeof te === "object" && te.families) {
    const structure = te.families.structure;
    if (structure && structure.observations) {
      const choch = structure.observations.find(o => o.type === "CHoCH");
      if (choch && choch.level != null) {
        invalidates.push({
          source: "TE Structure",
          description: "A confirmed break below " + choch.level + " (CHoCH level) would weaken the structural thesis",
          level: choch.level,
        });
      }
      const mtfConflict = structure.observations.find(o => o.type === "MTF_CONFLICT");
      if (mtfConflict) {
        invalidates.push({
          source: "TE Structure",
          description: "Multi-timeframe structural conflict — lower timeframe may be counter-trend to higher timeframe",
          level: null,
        });
      }
    }
  ...
  // C8 stop/invalidation level
  if (levels && levels.stopLoss != null) {
    invalidates.push({
      source: "C8 Trade Plan",
      description: "A break below stop level " + levels.stopLoss + " would invalidate the trade plan",
      level: levels.stopLoss,
    });
  }
  if (levels && levels.invalidation != null && levels.invalidation !== levels.stopLoss) {
    invalidates.push({
      source: "C8 Trade Plan",
      description: "Invalidation level at " + levels.invalidation,
      level: levels.invalidation,
    });
  }

  // TE bearish momentum
  if (te && te.families && te.families.momentum && te.families.momentum.direction === "BEARISH") {
    invalidates.push({
      source: "TE Momentum",
      description: "Momentum evidence is bearish — may conflict with bullish thesis",
      level: null,
    });
  }
...
// ============================================================================
// COUNTER-THESIS (Step 7)
// Explains what could make the current thesis wrong. Only based on actual
// evidence. Never fabricates scenarios.
// ============================================================================
function buildCounterThesis(sources, te, levels) {
  const parts = [];

  // TE-based counter-thesis
  if (te && typeof te === "object" && te.families) {
    const dir = te.overallDirection;

    // Check for momentum vs structure conflict
    const structDir = te.families.structure && te.families.structure.direction;
    const momDir = te.families.momentum && te.families.momentum.direction;
    if (structDir && momDir && structDir !== "UNAVAILABLE" && momDir !== "UNAVAILABLE" && structDir !== momDir) {
      parts.push("Although structure is " + structDir.toLowerCase() + ", momentum is " + momDir.toLowerCase() + " — this divergence could resolve against the primary thesis.");
    }
    ...
    if (structure && structure.observations) {
      const mtfConflict = structure.observations.find(o => o.type === "MTF_CONFLICT");
      if (mtfConflict) {
        parts.push("Lower timeframe structure is counter-trend to the higher timeframe — alignment is needed for conviction.");
      }
    }

    // Check for volume divergence
    const volDir = te.families.volume && te.families.volume.direction;
    if (dir === "BULLISH" && volDir === "BEARISH") {
      parts.push("Volume evidence is bearish despite bullish structure — volume confirmation is absent.");
    }
  }

  // C8-based counter-thesis
  if (levels && levels.stopLoss != null) {
    parts.push("A confirmed break below " + levels.stopLoss + " would weaken the current thesis.");
  }

  if (parts.length === 0) {
    parts.push("No specific counter-thesis evidence identified from available data.");
  }
...
function defaultExplanation(symbol) {
  return {
    ...
    technicalEvidenceExplanation: null,
    technicalEvidenceFamilies: null,
...
function buildAIExplanation(symbol, sources, opts = {}) {
  ...
  const te = opts.technicalEvidence || null;

  const availableCount = Object.values(sources).filter((v) => sourceHasData(v)).length;
  const hasTE = te && typeof te === "object";
  if (availableCount === 0 && !hasTE) {
    return {
      ...defaultResult,
      symbol,
      reasons: ["No intelligence sources or technical evidence provided for explanation"],
    };
  }
  ...
  // ---- Phase 25.10 Methodology Evidence (wired into live execution path) ----
  const horizonEx = c7 && c7.horizonEvidence ? explainHorizon(c7.horizonEvidence) : null;
  const classicalEx = c7 && c7.classicalEvidence ? explainClassical(c7.classicalEvidence) : null;
  const candlestickEx = c7 && c7.candlestickEvidence ? explainCandlestick(c7.candlestickEvidence) : null;
  const gammaEx = c7 && c7.gammaContext && c7.gammaContext.gammaAvailability === 'AVAILABLE' ? explainGamma(c7.gammaContext) : null;

  // ---- Step 7: Technical Evidence Explanation ----
  const teEx = explainTechnicalEvidenceFull(te);

  const explanations = [
    pennyEx, optionsEx, institutionalEx, swingEx,
    explosionEx, catalystEx, opportunityEx, tradePlanEx,
    horizonEx, classicalEx, candlestickEx, gammaEx,
  ].filter(Boolean);
  ...
  // Why-now factors (includes TE factors)
  const whyNow = buildWhyNow(sources, explanations, te);

  // Key levels
  const levels = buildKeyLevels(sources);

  // Conflicts (includes TE family conflicts)
  const conflicts = detectConflicts(sources, te);

  // Step 7: What confirms / invalidates / counter-thesis
  const confirms = buildConfirms(sources, te, levels);
  const invalidates = buildInvalidates(sources, te, levels);
  const counterThesis = buildCounterThesis(sources, te, levels);
  ...
  return {
    ...
    technicalEvidenceExplanation: teEx ? teEx.explanation : null,
    technicalEvidenceFamilies: teEx ? teEx.families : null,
    ...
    dataAvailability: {
      ...buildDataAvailability(sources),
      horizonEvidence: c7 && c7.horizonEvidence ? true : false,
      classicalEvidence: c7 && c7.classicalEvidence && c7.classicalEvidence.evidenceCompleteness > 0 ? true : false,
      candlestickEvidence: c7 && c7.candlestickEvidence && Array.isArray(c7.candlestickEvidence.patterns) && c7.candlestickEvidence.patterns.length > 0 ? true : false,
      gammaContext: c7 && c7.gammaContext && c7.gammaContext.gammaAvailability === 'AVAILABLE' ? true : false,
      technicalEvidence: hasTE ? true : false,
    },
    dataCompleteness: dataCompleteness != null ? dataCompleteness : 0,

    provenance,
    limitations: buildLimitations(sources),
    disclaimer: "C9 AI Explanation is derived only from existing B1-B6, C7, C8, and Technical Evidence outputs — not a buy/sell recommendation. No price levels, targets, or confidence values are fabricated. Missing evidence remains null.",
  };
}
