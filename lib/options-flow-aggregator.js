...
export function aggregateOptionsFlow(observations = [], options = {}) {
  ...
  const hasQuantitativeFlow = directional.some((item) =>
    (Number.isFinite(item.notional) && item.notional > 0) ||
    (Number.isFinite(item.contracts) && item.contracts > 0),
  );
  const weight = (item) => {
  ...
  return {
    ...
    direction,
    confidence,
    quantitativeFlow: hasQuantitativeFlow,
