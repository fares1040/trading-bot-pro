 ...
 * - finraVerified is an in-memory, per-instance flag.
 * - On serverless platforms (Vercel, AWS Lambda, etc.), each cold start
 *   creates a new instance with finraVerified = false.
 * - A successful /api/finra-verify call flips finraVerified to true only
 *   for the current instance. Subsequent requests to the same instance
 *   will see 'connected' in /api/health, but a new cold start will revert
 *   to 'configured-awaiting-verification'.
 * - This does NOT change trading/scoring logic — it only affects /api/health
 *   reporting. To persist verification across restarts, an external
 *   persistence layer (e.g., a database or KV store) would be required,
 *   but none has been introduced to avoid adding new infrastructure.
 */
...
//
// SERVERLESS LIMITATION:
// On serverless/edge platforms, each cold start resets this flag to false.
// Verification state does NOT persist across instances or restarts without
// an external persistence layer. No external persistence has been added
// to avoid introducing new infrastructure. Call /api/finra-verify to
// re-establish verification in a new instance.
...
export function getInstitutionalProviderStatus() {
  if (!isFinraConfigured()) {
    ...
    return {
      ...
      provider: 'finra',
      confidence: null,
      lastVerifiedAt: finraVerifiedAt || null,
      capabilities: {
...
/**
 * Reset FINRA verification state to unverified (useful for testing).
 * Does not affect trading logic or data caches.
 */
export function resetFinraVerification() {
  finraVerified = false;
  finraVerifiedAt = 0;
}