 ...
 *
 * BOS confirms the prevailing trend by breaking a structural swing level:
 *   - Bullish BOS: current price breaks above the last confirmed swing high
 *     while the prior structure shows higher highs (uptrend confirmation).
 *   - Bearish BOS: current price breaks below the last confirmed swing low
 *     while the prior structure shows lower lows (downtrend confirmation).
 *
 * Uses only the most recent candle (closes[length-1]) vs the last confirmed
 * swing high/low. No look-ahead bias — the swing points are confirmed (lookback
 * bars have elapsed).
 *
 * @param {Object} params
 * @param {number[]} closes
 * @param {Array} pivots - from buildPivots
 * @param {Object} classifiedPivots - output from classifyPivotSequences
 * @returns {{detected: boolean, direction: string|null, level: number|null, confidence: number|null, description: string}}
 */
function detectBOS({ closes, pivots, classifiedPivots }) {
  const result = { detected: false, direction: null, level: null, confidence: null, description: 'No break of structure detected' };

  if (!Array.isArray(closes) || closes.length < 2) return result;
  if (!Array.isArray(pivots) || pivots.length < 2) return result;

  const currentPrice = n(closes[closes.length - 1]);
  ...
  if (confirmedHighs.length === 0 || confirmedLows.length === 0) return result;

  const lastConfirmedHigh = confirmedHighs[confirmedHighs.length - 1];
  const lastConfirmedLow = confirmedLows[confirmedLows.length - 1];

  // Determine prevailing trend from classified pivots
  const recentClassified = classifiedPivots.filter(p => p.classification != null).slice(-6);
  const hhCount = recentClassified.filter(p => p.classification === 'HH').length;
  const llCount = recentClassified.filter(p => p.classification === 'LL').length;

  // Bullish BOS: price breaks above last swing high while prior structure shows HH (uptrend)
  if (currentPrice > lastConfirmedHigh.price && hhCount > llCount) {
    return {
      detected: true,
      direction: 'BULLISH',
      level: lastConfirmedHigh.price,
      confidence: clamp(60 + (hhCount - llCount) * 10, 60, 95),
      description: `Bullish BOS: price ${currentPrice} broke above swing high ${lastConfirmedHigh.price}`,
      brokenLevel: lastConfirmedHigh.price,
      breakPrice: currentPrice,
      brokenAtIndex: lastConfirmedHigh.index,
    };
  }

  // Bearish BOS: price breaks below last swing low while prior structure shows LL (downtrend)
  if (currentPrice < lastConfirmedLow.price && llCount > hhCount) {
    return {
      detected: true,
      direction: 'BEARISH',
      level: lastConfirmedLow.price,
      confidence: clamp(60 + (llCount - hhCount) * 10, 60, 95),
      description: `Bearish BOS: price ${currentPrice} broke below swing low ${lastConfirmedLow.price}`,
      brokenLevel: lastConfirmedLow.price,
      breakPrice: currentPrice,
      brokenAtIndex: lastConfirmedLow.index,
    };
  }
 ...
 *
 * CHoCH signals a potential trend reversal by breaking structure against
 * the prevailing trend:
 *   - Bullish CHoCH: in a downtrend (LL structure), price breaks above the
 *     last lower high — first sign of bullish reversal.
 *   - Bearish CHoCH: in an uptrend (HH structure), price breaks below the
 *     last higher low — first sign of bearish reversal.
 *
 * CHoCH is distinct from BOS:
 *   - BOS confirms the existing trend
 *   - CHoCH contradicts the existing trend
 *
 * Both cannot fire on the same bar for the same direction.
 *
 * @param {Object} params
 * @param {number[]} closes
 * @param {Array} pivots - from buildPivots
 * @param {Array} classifiedPivots - from classifyPivotSequences
 * @returns {{detected: boolean, direction: string|null, level: number|null, confidence: number|null, description: string}}
 */
function detectCHoCH({ closes, pivots, classifiedPivots }) {
  const result = { detected: false, direction: null, level: null, confidence: null, description: 'No change of character detected' };
  ...
  // Bullish CHoCH: in downtrend (more LL than HH), price breaks above last lower high
  if (currentPrice > lastConfirmedHigh.price && llCount > hhCount) {
    return {
      detected: true,
      direction: 'BULLISH',
      level: lastConfirmedHigh.price,
      confidence: clamp(55 + (llCount - hhCount) * 10, 55, 90),
      description: `Bullish CHoCH: price ${currentPrice} broke above last lower high ${lastConfirmedHigh.price} in downtrend`,
      brokenLevel: lastConfirmedHigh.price,
      breakPrice: currentPrice,
      brokenAtIndex: lastConfirmedHigh.index,
    };
  ...
  // Bearish CHoCH: in uptrend (more HH than LL), price breaks below last higher low
  if (currentPrice < lastConfirmedLow.price && hhCount > llCount) {
    return {
      detected: true,
      direction: 'BEARISH',
      level: lastConfirmedLow.price,
      confidence: clamp(55 + (hhCount - llCount) * 10, 55, 90),
      description: `Bearish CHoCH: price ${currentPrice} broke below last higher low ${lastConfirmedLow.price} in uptrend`,
      brokenLevel: lastConfirmedLow.price,
      breakPrice: currentPrice,
      brokenAtIndex: lastConfirmedLow.index,
    };
...
function calculateStructureScore(inputs) {
  ...
  if (seqCount > 0) {
    components.push({ score: clamp(seqCount * 10, 0, 100), weight: 0.30 });
  }

  const trendConf = inputs.trendConfidence;
  if (trendConf != null) {
    components.push({ score: trendConf, weight: 0.20 });
  }

  if (inputs.mssDetected) {
    components.push({ score: 75, weight: 0.10 });
  }

  if (inputs.breakerDetected) {
    components.push({ score: 60, weight: 0.08 });
  }

  const unfilledFVG = inputs.unfilledFvgCount || 0;
  if (unfilledFVG > 0) {
    components.push({ score: clamp(unfilledFVG * 15, 0, 100), weight: 0.08 });
  }

  // BOS confirms trend — strong structural evidence
  if (inputs.bosDetected) {
    components.push({ score: 80, weight: 0.14 });
  }

  // CHoCH signals reversal — significant structural event
  if (inputs.chochDetected) {
    components.push({ score: 70, weight: 0.10 });
  }
...
function classifyStructureState(inputs) {
  const { trend, higherHighs, higherLows, lowerHighs, lowerLows, mssDetected, bosDetected, bosDirection, chochDetected, chochDirection } = inputs;

  if (trend === 'UNAVAILABLE') return STRUCTURE_STATE.INSUFFICIENT_DATA;

  // CHoCH always signals TRANSITION (reversal in progress)
  if (chochDetected) return STRUCTURE_STATE.TRANSITION;

  // BOS confirms the prevailing trend
  if (bosDetected && bosDirection === 'BULLISH') return STRUCTURE_STATE.BULLISH_CONTINUATION;
  if (bosDetected && bosDirection === 'BEARISH') return STRUCTURE_STATE.BEARISH_CONTINUATION;
...
function buildStructureSignals(inputs) {
  ...
  // BOS signal (highest priority — confirms trend)
  if (inputs.bosDetected) {
    signals.push({
      type: 'BOS',
      direction: inputs.bosDirection,
      level: inputs.bosLevel,
      breakPrice: inputs.bosBreakPrice,
      confidence: inputs.bosConfidence,
      description: inputs.bosDescription || `Break of Structure: ${inputs.bosDirection}`,
      family: 'STRUCTURE',
    });
  }

  // CHoCH signal (second priority — signals reversal)
  if (inputs.chochDetected) {
    signals.push({
      type: 'CHoCH',
      direction: inputs.chochDirection,
      level: inputs.chochLevel,
      breakPrice: inputs.chochBreakPrice,
      confidence: inputs.chochConfidence,
      description: inputs.chochDescription || `Change of Character: ${inputs.chochDirection}`,
      family: 'STRUCTURE',
    });
  }
...
function buildStructureIntelligence(inputs = {}) {
  ...
  // --- BOS / CHoCH detection (requires pivots + classified pivots + current price) ---
  const bos = detectBOS({ closes: c, pivots, classifiedPivots });
  const choch = detectCHoCH({ closes: c, pivots, classifiedPivots });
  ...
  const signals = buildStructureSignals({
    ...
    bosDirection: bos.direction,
    bosLevel: bos.level,
    bosBreakPrice: bos.breakPrice,
    bosConfidence: bos.confidence,
    bosDescription: bos.description,
    chochDetected: choch.detected,
    chochDirection: choch.direction,
    chochLevel: choch.level,
    chochBreakPrice: choch.breakPrice,
    chochConfidence: choch.confidence,
    chochDescription: choch.description,
    mssDetected: marketStructure.mssDetected,
    lastStructureShift: marketStructure.lastStructureShift,
    mssTimestamp: null,
  ...
  return {
    ...
    dataQuality: {
      ...
      hasBOS: bos.detected,
      hasCHoCH: choch.detected,
    },
    dataStatus: 'complete',
    provenance: {
      pivots: 'structure-intelligence-manager',
      classification: 'structure-intelligence-manager',
      bos: 'structure-intelligence-manager',
      choch: 'structure-intelligence-manager',
...
function defaultStructureIntelligence(symbol = null, timeframe = TIME_IN_FORCE.DAILY) {
  return {
    ...
    bos: { detected: false, direction: null, level: null, confidence: null, description: 'No break of structure detected' },
    choch: { detected: false, direction: null, level: null, confidence: null, description: 'No change of character detected' },
