...
const MARKET_DATA_STALE_THRESHOLD_MS = 48 * 60 * 60 * 1000;
import { recordProviderFailure } from './circuit-breaker-manager.js';
function nowISO(){ return new Date().toISOString(); }

export const yahooHeaders = YAHOO_HEADERS;
export const SYMBOL_RE = /^[A-Z][A-Z0-9.^=-]{0,11}$/;
const FALLBACK_DISCOVERY_SYMBOLS = ['AMD','PLTR','SOFI','NIO','HIMS','SOUN','RIVN','IONQ','F','BAC','AAPL','MSFT','NVDA','TSLA','AMZN','META','INTC','PYPL','SNAP','PFE'];
...
export async function fetchChart(symbol,range='6mo'){
  ...
  let res;
  try {
    res = await fetch(`https://query1.finance.yahoo.com/v8/finance/chart/${encodeURIComponent(clean)}?interval=1d&range=${range}&events=div%2Csplits`,{headers:YAHOO_HEADERS,cache:'no-store',signal:AbortSignal.timeout(10000)});
  } catch (error) {
    recordProviderFailure('yahoo', error, 'market-data', clean, false);
    throw error;
  }
  if (!res.ok) {
    const status = res.status;
    if (status === 429) {
      recordProviderFailure('yahoo', new Error('Yahoo Finance rate limit'), 'market-data', clean, false);
      throw new Error('Yahoo Finance rate limit');
    }
    recordProviderFailure('yahoo', new Error(`Yahoo Finance ${status}`), 'market-data', clean, false);
    throw new Error(`Yahoo Finance ${status}`);
  }
  const json=await res.json(), result=json?.chart?.result?.[0];
  if(!result?.meta){
    recordProviderFailure('yahoo', new Error('No market data'), 'market-data', clean, false);
    throw new Error('No market data');
  }
  const lastYahooTs = result.timestamp?.[result.timestamp.length-1]??null;
  const asOf=lastYahooTs?new Date(lastYahooTs*1000).toISOString():null;
  const fetchedAt=nowISO();
  const stale=asOf?(new Date(fetchedAt)-new Date(asOf)>MARKET_DATA_STALE_THRESHOLD_MS):false;
  return {symbol:result.meta.symbol||clean,meta:result.meta,quote:result.indicators?.quote?.[0]||{},timestamps:result.timestamp||[],asOf,fetchedAt,stale};
}
export async function fetchTrending(count=25){
  const fallback = () => {
    const configured = String(process.env.HUNTER_UNIVERSE || '').split(',').map((symbol) => symbol.trim().toUpperCase()).filter((symbol) => SYMBOL_RE.test(symbol));
    return [...new Set([...configured, ...FALLBACK_DISCOVERY_SYMBOLS])].slice(0, Math.max(count, 20));
  };
  try {
    const res=await fetch(`https://query1.finance.yahoo.com/v1/finance/trending/US?count=${count}`,{headers:YAHOO_HEADERS,cache:'no-store',signal:AbortSignal.timeout(10000)});
    if(!res.ok){recordProviderFailure('yahoo',new Error(`Yahoo trending ${res.status}`),'market-data',null,false);return fallback();}
    const j=await res.json();
    const symbols=(j?.finance?.result?.[0]?.quotes||[]).map(x=>x?.symbol).filter(x=>SYMBOL_RE.test(x||''));
    return symbols.length ? symbols : fallback();
  } catch(error) {
    recordProviderFailure('yahoo',error,'market-data',null,false);
    return fallback();
  }
}
