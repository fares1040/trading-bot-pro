...
export async function fetchTrending(count=25){let res;try{res=await fetch(`https://query1.finance.yahoo.com/v1/finance/trending/US?count=${count}`,{headers:YAHOO_HEADERS,cache:'no-store',signal:AbortSignal.timeout(10000)});}catch(error){recordProviderFailure('yahoo',error,'market-data',null,false);throw error;}if(!res.ok){const status=res.status;if(status===429){recordProviderFailure('yahoo',new Error('Yahoo Trending rate limit'),'market-data',null,false);throw new Error('Yahoo Trending rate limit');}recordProviderFailure('yahoo',new Error(`Yahoo trending ${status}`),'market-data',null,false);throw new Error(`Yahoo trending ${status}`);}const j=await res.json();return (j?.finance?.result?.[0]?.quotes||[]).map(x=>x?.symbol).filter(x=>SYMBOL_RE.test(x||''));}
