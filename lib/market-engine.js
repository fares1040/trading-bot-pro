...
export async function fetchChart(symbol,range='6mo'){
  ...
  const res=await fetch(`https://query1.finance.yahoo.com/v8/finance/chart/${encodeURIComponent(clean)}?interval=1d&range=${range}&events=div%2Csplits`,{headers:YAHOO_HEADERS,cache:'no-store',signal:AbortSignal.timeout(10000)}); if(!res.ok)throw new Error(`Yahoo Finance ${res.status}`); const json=await res.json(), result=json?.chart?.result?.[0]; if(!result?.meta)throw new Error('No market data');   const lastYahooTs = result.timestamp?.[result.timestamp.length-1]??null;
   return {symbol:result.meta.symbol||clean,meta:result.meta,quote:result.indicators?.quote?.[0]||{},timestamps:result.timestamp||[],asOf:lastYahooTs?new Date(lastYahooTs*1000).toISOString():null,fetchedAt:nowISO()};
 }
export async function fetchTrending(count=25){const res=await fetch(`https://query1.finance.yahoo.com/v1/finance/trending/US?count=${count}`,{headers:YAHOO_HEADERS,cache:'no-store',signal:AbortSignal.timeout(10000)});if(!res.ok)throw new Error(`Yahoo trending ${res.status}`);const j=await res.json();return (j?.finance?.result?.[0]?.quotes||[]).map(x=>x?.symbol).filter(x=>SYMBOL_RE.test(x||''));}
