...
function hasQuantitativeFlow(observations) {
  return observations.some((item) => {
    const notional = Number(item?.notional);
    const contracts = Number(item?.contracts);
    return (Number.isFinite(notional) && notional > 0) || (Number.isFinite(contracts) && contracts > 0);
  });
}
...
export function classifyOptionsSmartMoney(observations = [], options = {}) {
  ...
  const quantitativeFlow = hasQuantitativeFlow(observations);

  let classification = 'UNATTRIBUTED';
  if (explicit.length > 0 && freshCount > 0 && aggregate.direction !== 'UNKNOWN') classification = 'CONFIRMED';
  else if (freshCount >= 3 && quantitativeFlow && aggregate.pressureScore >= 65) classification = 'POSSIBLE';

  const score = Math.min(100, (aggregate.pressureScore || 0) * 0.5 + Math.min(30, freshCount * 6) + (explicit.length > 0 ? 20 : 0));
