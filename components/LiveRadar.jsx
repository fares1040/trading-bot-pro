...
function StatusBadge({ status }) { const color = STATUS_COLORS[status] || '#6B7280'; return <span style={{ fontSize: 9, padding: '3px 8px', borderRadius: 9999, backgroundColor: color + '20', border: `1px solid ${color}40`, color, fontWeight: 700 }}>{status}</span>; }
function DirectionBadge({ direction }) { const color = DIRECTION_COLORS[direction] || '#6B7280'; return <span style={{ fontSize: 10, color, fontWeight: 700 }}>{direction === 'LONG' ? '↑' : direction === 'SHORT' ? '↓' : '—'} {direction}</span>; }
...
function PressureBar({ pressure }) { if (pressure == null) return <span style={{ fontSize: 10, color: colors.text.muted }}>—</span>; const color = pressure >= 65 ? '#22C55E' : pressure <= 35 ? '#EF4444' : '#FBBF24'; return <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}><div style={{ width: 60, height: 6, backgroundColor: '#1F2636', borderRadius: 3 }}><div style={{ width: `${Math.max(0, Math.min(100, pressure))}%`, height: '100%', backgroundColor: color, borderRadius: 3 }} /></div><span style={{ fontSize: 10, color, fontWeight: 700 }}>{pressure}</span></div>; }
...
function OpportunityRow({ entry }) {
  ...
  return <div style={{ padding: '12px 14px', borderRadius: radius.md, backgroundColor: '#0B0F17D0', border: '1px solid #1F263660', display: 'flex', flexDirection: 'column', gap: 8 }}>
    <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: 8 }}><div style={{ display: 'flex', alignItems: 'center', gap: 10 }}><span style={{ color: colors.text.primary, fontWeight: 900, fontSize: 13 }}>{entry.symbol}</span><DirectionBadge direction={entry.opportunity?.direction || 'NEUTRAL'} /><StatusBadge status={entry.opportunity?.status || entry.status} /></div><div style={{ display: 'flex', alignItems: 'center', gap: 12 }}><FreshnessBadge freshness={entry.freshness} /><PressureBar pressure={entry.pulse?.pressure} /></div></div>
    ...
    {entry.acceleration?.direction && entry.acceleration.direction !== 'UNAVAILABLE' && <div style={{ fontSize: 10, color: colors.text.muted }}>Acceleration: {entry.acceleration.direction}{entry.acceleration.historySize ? ` (${entry.acceleration.historySize} samples)` : ''}</div>}
...
export default function LiveRadar() {
  const [data, setData] = useState([]); const [loading, setLoading] = useState(true); const [error, setError] = useState(null); const [symbols, setSymbols] = useState('NVDA,AMD,TSLA'); const intervalRef = useRef(null);
  const fetchData = useCallback(async (syms) => { try { const res = await fetch(`/api/live-opportunities?symbols=${encodeURIComponent(syms)}`, { cache: 'no-store' }); const json = await res.json(); if (!json.success) { setError(json.errors?.[0]?.error || 'Failed to load'); setData([]); } else { setData(json.data || []); setError(null); } } catch (err) { setError(err?.message || 'Network error'); setData([]); } finally { setLoading(false); } }, []);
  useEffect(() => { fetchData(symbols); intervalRef.current = setInterval(() => fetchData(symbols), 20000); return () => clearInterval(intervalRef.current); }, [symbols, fetchData]);
  const handleSearch = () => { setLoading(true); clearInterval(intervalRef.current); fetchData(symbols); intervalRef.current = setInterval(() => fetchData(symbols), 20000); };
