...
export default function LiveRadarShell(){
  ...
  const topDecisions=useMemo(()=>liveData.filter(entry=>entry?.freshness==='FRESH'&&['IGNITION','PREPARING','OPPORTUNITY','WATCH'].includes(entry?.earlyExplosion?.state)||entry?.status==='OPPORTUNITY'||entry?.status==='WATCH').sort((a,b)=>rankLiveEntry(b)-rankLiveEntry(a)).slice(0,3),[liveData]);
