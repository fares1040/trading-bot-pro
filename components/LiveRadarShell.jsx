...
export default function LiveRadarShell(){
  ...
  const handleData=useCallback((nextData)=>{setLiveData(Array.isArray(nextData)?nextData:[]);setLastUpdate(new Date());},[]);
  ...
  const updateLabel=lastUpdate?lastUpdate.toLocaleTimeString('ar-SA',{hour:'2-digit',minute:'2-digit',second:'2-digit'}):'بانتظار أول تحديث';
  return <div className="live-radar-shell" dir="rtl">
    ...
    <div className="live-radar-meta"><span>{summary.total} سهم مفحوص</span><span>·</span><span>{summary.opportunities} فرص</span><span>·</span><span>{summary.watches} مراقبة</span><span>·</span><span>{summary.freshRatio.toFixed(0)}% بيانات حديثة</span><span>·</span><span>آخر تحديث: {updateLabel}</span></div>
